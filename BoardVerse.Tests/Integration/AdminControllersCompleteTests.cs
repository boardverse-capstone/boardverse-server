...
/// <summary>
/// Tests for PaymentMasterAccountController
/// </summary>
[Collection(IntegrationTestCollection.Name)]
public class PaymentMasterAccountControllerIntegrationTests
{
    private readonly HttpClient _client;

    public PaymentMasterAccountControllerIntegrationTests(BoardVerseWebApplicationFactory factory) =>
        _client = factory.CreateClient();

    #region === MASTER ACCOUNT ===

    [IntegrationFact]
    public async Task MasterAccount_GetAll()
    {
        var token = await IntegrationTestAuth.AsAdminAsync(_client);
        ApiTestClient.Authorize(_client, token);
    ...
    [IntegrationFact]
    public async Task MasterAccount_GetById()
    {
        var token = await IntegrationTestAuth.AsAdminAsync(_client);
        ApiTestClient.Authorize(_client, token);
    ...
    [IntegrationFact]
    public async Task MasterAccount_Create()
    {
        var token = await IntegrationTestAuth.AsAdminAsync(_client);
        ApiTestClient.Authorize(_client, token);
        ...
        {
            provider = "SePay",
            accountHolder = $"Test Account {Guid.NewGuid():N}".Substring(0, 20),
            bankCode = "TPBANK",
    ...
    [IntegrationFact]
    public async Task MasterAccount_Update()
    {
        var token = await IntegrationTestAuth.AsAdminAsync(_client);
        ApiTestClient.Authorize(_client, token);
    ...
    [IntegrationFact]
    public async Task MasterAccount_Delete()
    {
        var token = await IntegrationTestAuth.AsAdminAsync(_client);
        ApiTestClient.Authorize(_client, token);
