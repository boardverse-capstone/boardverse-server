...
public class FriendServiceTests
{
    ...
    #region GetOnlineFriendsAsync Tests

    [Fact]
    public async Task GetOnlineFriendsAsync_ReturnsOnlyFriendsWithin5Minutes()
    {
        var userId = Guid.NewGuid();
        var onlineFriendId = Guid.NewGuid();
        var offlineFriendId = Guid.NewGuid();

        var onlineFriend = new Friendship
        {
            Id = Guid.NewGuid(),
            RequesterId = userId,
            AddresseeId = onlineFriendId,
            Status = FriendshipStatus.Accepted,
            Requester = null!,
            Addressee = new User
            {
                ...
                Profile = new UserProfile
                {
                    ...
                    LastActiveAt = DateTime.UtcNow.AddMinutes(-2) // within 5 minutes
                }
            }
        };

        var offlineFriend = new Friendship
        {
            Id = Guid.NewGuid(),
            RequesterId = userId,
            AddresseeId = offlineFriendId,
            Status = FriendshipStatus.Accepted,
            Requester = null!,
            Addressee = new User
            {
                ...
                Profile = new UserProfile
                {
                    UserId = offlineFriendId,
                    LastActiveAt = DateTime.UtcNow.AddMinutes(-10) // older than 5 minutes
                }
            }
        };

        _friendshipRepo.Setup(r => r.GetFriendsAsync(userId))
            .ReturnsAsync(new List<Friendship> { onlineFriend, offlineFriend });

        var svc = CreateService();
        var result = await svc.GetOnlineFriendsAsync(userId);

        Assert.Single(result);
        Assert.Equal(onlineFriendId, result[0].UserId);
        Assert.Equal("online_friend", result[0].Username);
    }

    [Fact]
    public async Task GetOnlineFriendsAsync_NoOnlineFriends_ReturnsEmptyList()
    {
        var userId = Guid.NewGuid();
    ...
    [Fact]
    public async Task GetOnlineFriendsAsync_MultipleOnlineFriends_ReturnsAllOnlineFriends()
    {
        var userId = Guid.NewGuid();
        ...
        var friend2 = new Friendship
        {
            Id = Guid.NewGuid(),
            RequesterId = userId,
            AddresseeId = friend2Id,
            Status = FriendshipStatus.Accepted,
            Requester = null!,
            Addressee = new User
            {
                ...
                {
                    UserId = friend2Id,
                    LastActiveAt = DateTime.UtcNow.AddMinutes(-3)
                }
            }
        };
        ...
        _friendshipRepo.Setup(r => r.GetFriendsAsync(userId))
            .ReturnsAsync(new List<Friendship> { friend1, friend2, offlineFriend });

        var svc = CreateService();
        var result = await svc.GetOnlineFriendsAsync(userId);

        Assert.Equal(2, result.Count);
        Assert.Contains(result, f => f.UserId == friend1Id);
        Assert.Contains(result, f => f.UserId == friend2Id);
        Assert.DoesNotContain(result, f => f.UserId == offlineFriendId);
    }

    [Fact]
    public async Task GetOnlineFriendsAsync_HandlesNullProfile()
    {
        var userId = Guid.NewGuid();
        var friendId = Guid.NewGuid();

        var friendWithNullProfile = new Friendship
        {
            Id = Guid.NewGuid(),
            RequesterId = userId,
            AddresseeId = friendId,
        ...
        _friendshipRepo.Setup(r => r.GetFriendsAsync(userId))
            .ReturnsAsync(new List<Friendship> { friendWithNullProfile });

        var svc = CreateService();
        var result = await svc.GetOnlineFriendsAsync(userId);

        Assert.Empty(result); // Should skip users with null LastActiveAt
    }
